...
export function TrainingManagement() {
  const [trainingsList, setTrainingsList] = useState(initialTrainings);
     useEffect(() => {
        fetch("http://localhost:8080/api/treinamentos")
            .then((resposta) => resposta.json())
            .then((dadosReais) => {
                // Se a API retornar dados válidos, substituímos a lista falsa!
                if (dadosReais && dadosReais.length > 0) {
                    setTrainingsList(dadosReais);
                }
            })
            .catch((erro) => console.error("Erro ao conectar com o Go:", erro));
    }, []);
  ...
  const handleSuccess = (formData: any) => {
    function formatToBrazilianDate(dateString: string) {
    ...
    if (editingTraining) {
        ...
        prev.map((t) => {
          if (t.id === editingTraining.id) {
            return {
              ...
              data: formatToBrazilianDate(formData.data),
              hora: formData.horarioInicio || "00:00",
              dataHora: `${formData.data}T${formData.horarioInicio || "00:00"}:00`,
              conteudo: formData.conteudo,
              isCancelado: formData.status === "cancelado",
            };
          }
          return t;
        })
      ...
      setTrainingsList((prev) => [novoTreinamento, ...prev]);
    }
